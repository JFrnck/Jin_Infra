...
# Requiere: flux CLI instalado y GITHUB_TOKEN. Usá un PAT **fine-grained**
# acotado a ESTE repo (Contents: R/W, Administration: R/W para la deploy key,
# Metadata: R) y con caducidad corta -- NO el classic `repo`, que da control
# total sobre todos los repos privados del owner. El token se usa una sola vez:
# después Flux trabaja con la deploy key que deja creada.
set -euo pipefail
...
if [[ -z "${GITHUB_TOKEN:-}" ]]; then
  echo "ERROR: exporta GITHUB_TOKEN (PAT fine-grained acotado a Jin_Infra) antes de correr esto." >&2
  exit 1
fi
